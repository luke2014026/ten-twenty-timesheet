...
const shellQuote__default = /*#__PURE__*/_interopDefaultLegacy(shellQuote);
...
function getArgumentsForLineNumber(editor, fileName, lineNumber, colNumber, workspace) {
  const editorBasename = path__default.basename(editor).replace(/\.(exe|cmd|bat)$/i, "");
...
function guessEditor() {
  ...
  if (process.env.REACT_EDITOR)
    return shellQuote__default.parse(process.env.REACT_EDITOR);
  try {
    if (process.platform === "darwin") {
      const output = child_process__default.execSync("ps x", { stdio: ["pipe", "pipe", "ignore"] }).toString();
      ...
      const output = child_process__default.execSync('powershell -Command "Get-Process | Select-Object Path"', {
      ...
      for (let i = 0; i < runningProcesses.length; i++) {
        ...
        const shortProcessName = path__default.basename(fullProcessPath);
        if (COMMON_EDITORS_WIN.includes(shortProcessName))
          return [fullProcessPath];
      }
    } else if (process.platform === "linux") {
      const output = child_process__default.execSync("ps x --no-heading -o comm --sort=comm", { stdio: ["pipe", "pipe", "ignore"] }).toString();
...
function printInstructions(fileName, errorMessage) {
  console.log();
  console.log(chalk__default.red(`Could not open ${path__default.basename(fileName)} in the editor.`));
  if (errorMessage) {
    if (errorMessage[errorMessage.length - 1] !== ".")
      errorMessage += ".";
    console.log(chalk__default.red(`The editor process exited with an error: ${errorMessage}`));
  }
  console.log();
  console.log(`To set up the editor integration, add something like ${chalk__default.cyan("REACT_EDITOR=atom")} to the ${chalk__default.green(".env.local")} file in your project folder and restart the development server. Learn more: ${chalk__default.green("https://goo.gl/MMTaZt")}`);
  console.log();
}
...
function launchEditor(fileName, lineNumber, colNumber) {
  ...
  if (editor.toLowerCase() === "none")
    return;
  if (process.platform === "linux" && fileName.startsWith("/mnt/") && /Microsoft/i.test(os__default.release())) {
    fileName = path__default.relative("", fileName);
  }
  ...
  if (process.platform === "win32") {
    _childProcess = child_process__default.spawn("cmd.exe", ["/C", editor].concat(args), { stdio: "inherit" });
  } else {
    _childProcess = child_process__default.spawn(editor, args, { stdio: "inherit" });
  }
...
function VitePluginReactInspector() {
  return {
...
module.exports = VitePluginReactInspector;
